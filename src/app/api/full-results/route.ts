/**
 * Full live results: fans out to all 234 candidateswise-S22{ac}.htm pages,
 * parses each, aggregates vote counts and margins. This is what powers the
 * Insights page, Parties vote share column, and the close-contest sort.
 *
 * Cost: 234 ECI fetches per refresh. We batch in groups of 24 with a 30s
 * server-side cache, so a popular page hammering /api/results doesn't fan
 * out 234 requests every 30 seconds — only the first request after the
 * cache expires does the work, and the rest get the cached payload.
 */
...
async function fetchAllAcs(): Promise<Map<number, EciConstituency>> {
...
function buildLiveResults(byAc: Map<number, EciConstituency>): LiveResults {
  ...
  const constituencies = CONSTITUENCIES.map((c) => {
    ...
    const status = eci.leader.status === "won" ? "won" : eci.leader.status === "trailing" ? "leading" : eci.leader.status;
    if (status === "won") {
    ...
    return {
      acNo: c.id,
      status: (status === "won" ? "won" : "leading") as "won" | "leading",
...
export async function GET() {
  ...
  const liveResults = buildLiveResults(byAc);
  return NextResponse.json(liveResults, {
    headers: {
      "Cache-Control": "s-maxage=30, stale-while-revalidate=60",
      "X-Source": "eci-live-html-fanout",
